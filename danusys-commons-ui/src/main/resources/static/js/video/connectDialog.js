...
let dialogManager = {
	...
	init : function(mapManager) {
		...
		this.layer = this.mapManager.createVectorLayer('connectLine', this.connectLineStyle, source);

		try {
			this.mapManager.setMapEventListener('postrender', function(e){
	...
	mapMoveEvent : function() {
		this.list.forEach(function(val, key, map) {
			...
			dialogManager.setLineFeatureGeom(data, lineFeature, left, top);
		});
	},
	/**
	...
	sortDialog : function() {
		...
		this.list.forEach(function(val, key, map) {
			...
			dialogManager.setDialogPosition(val, elTop, elLeft, top, left);
			count++;
		});
	},
	/**
	...
	createDialog : function(option) {
		...
		const dialog = $('<div>').addClass('dialog').css(option.css).bind({
			'mouseover' : function() {
				dialogManager.setFeatureStyle(lineFeature, '#FF0000');
			},
			'mouseleave' : function() {
				if($(dialog).hasClass('active')) return;
				dialogManager.setFeatureStyle(lineFeature, color);
			},
			'click' : function(e) {
				const selectedOverlay = dialogManager.setDialogSelectedOverlay(fPoint);
		...
		this.list.set(option.data.facilitySeq, dialog);
	...
	setDialogSelectedOverlay : function (position) {
		const content = dialogManager.createDialogSelectedOverlay();
		const option = {
			id : 'dialogSelected',
			position : position,
			element : content,
		...
		return this.mapManager.setOverlay(option);
	},
	...
	close : function(dialog) {
		this.mapManager.removeOverlay(dialog.data('selectedOverlay'));
		this.layer.getSource().removeFeature(dialog.data('lineFeature'));
		this.list.delete(dialog.data('option').data.facilitySeq);
		dialog.remove();
	...
	closeAll : function() {
		...
		this.mapManager.removeOverlayById('dialogSelected');
	}
}
