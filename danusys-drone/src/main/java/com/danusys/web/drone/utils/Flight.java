...
public class Flight {
    ...
    public MavlinkConnection takeoff(float takeOffAlt) {
...
//armed
            ...
            while ((message = connection.next()) != null) {
                ...
                if (message.getPayload().getClass().getName().contains("GlobalPositionInt")) {
                    this.simpMessagingTemplate.convertAndSend("/topic/drone3", gson.toJson(message.getPayload()));
                ...
                if (message.getPayload().getClass().getName().contains("TerrainReport")) {
                    ...
                    if (currentHeightFloat >= takeOffAlt - 1.1) {
                        break;
                    }
                }
...
//mavmodetype
            ...
            System.out.println("전송됨0");
        }

        return connection;
    }

    public MavlinkConnection wayPoint(float x, float y, float z) {
        MavlinkConnection connection = null;
        try (Socket socket = new Socket("172.20.14.87", 14550)) {
...
//            connection.send1(255, 0, STATUS_gps);
    ...
    public MavlinkConnection returnDrone() {
        MavlinkConnection connection = null;
