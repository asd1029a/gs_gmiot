		...
		$("#modAdminPwdBtn").on('click', () => {
			...
			if(typeof adminObj != "undefined") {
				if(adminObj.useFlag !== "2") {
					...
					$('#adminPwdForm input[name="adminId"]').val(adminObj.adminId);
					$('#adminPwdForm input[name="adminSeq"]').val(adminObj.adminSeq);
				} else {
...
const adminGroup = {
	init : () => {
		...
		$("#addAdminInGroupBtn").on('click', () => {
			...
			if(typeof selectedData !== "undefined" && selectedData.adminGroupSeq !== "" && selectedData.adminGroupSeq !== null) {
				adminGroup.showAdminInGroupPopup(selectedData.adminGroupSeq);
			} else {
		...
		$("#getListAdminInGroupBtn").on('click', ()=> {
			const adminGroupSeq = $('#adminGroupTable').DataTable().row('.selected').data().adminGroupSeq;
		...
		const optionObj = {
			...
			const evt = {
				click : function(e) {
					...
					adminGroup.getListAdminInGroup($('#adminInGroupTable'), rowData.adminGroupSeq);
					if($(e.target).hasClass('tableButton')) {
	...
	getListAdminInGroup : ($target, pAdminGroupSeq) => {
		const optionObj = {
	            ...
	            ajax : 
	            	{
						...
						'type' : "POST",
						'data' : function ( d ) {
							d.adminGroupSeq = pAdminGroupSeq
							return JSON.stringify( d );
						},
						'dataSrc' : function (result) {
			...
			const evt = {
					click : function(e) {
						const adminSeq = $target.DataTable().row($(e.currentTarget)).data().adminSeq;
						const adminGroupSeq = $('#adminGroupTable').DataTable().row('.selected').data().adminGroupSeq;
	...
	getListAdminInGroupCheck : ($target, pAdminGroupSeq) => {
		const optionObj = {
		        ...
		        "fnCreatedRow": (nRow, aaData, iDataIndex) => {
		        	$(nRow).find('input').prop('id', "check"+aaData.adminSeq);
		        	$(nRow).find('input').prop('value', aaData.adminSeq);
		        	$(nRow).find('label').prop('for', "check"+aaData.adminSeq);
		        	if(aaData.checked === 'checked' || $('#adminInGroupPopup').data('adminSeqList').indexOf(aaData.adminSeq) > -1) {
	...
	delAdminGroupProc : () => {
		comm.ajaxPost({
			...
			, type : "DELETE"
			, data : {adminGroupSeq : $("#adminGroupSeq").val()}
		},
		(result) => {
			adminGroup.getListAdminGroup($('#adminGroupTable'));
			adminGroup.getListAdminInGroup($('#adminInGroupTable'), "0");
	...
	delAdminInGroupProc : (pAdminSeq, pAdminGroupSeq) => {
		comm.ajaxPost({
			...
			, type : "DELETE"
			, data : {adminGroupSeq : pAdminGroupSeq
					  ,	adminSeq : pAdminSeq}
		},
		(result) => {
