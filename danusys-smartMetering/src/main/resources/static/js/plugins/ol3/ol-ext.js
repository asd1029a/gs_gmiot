...
{	// Nothing selected
	...
	var ajaxObj=[];
	cluster.forEach(function(eFeature){
		var info = eFeature.getProperties();	
		if(info.modifyYn=="Y"){
			ajaxObj.push(info.fcltId);
		}
	});
	var coords;
	if(ajaxObj.length>0){
		comm.ajaxPost({
			url : '/cctv/getPresetCoord.ado',
			"showloading" : false,
			data: {"fcltIds":ajaxObj},
			async: false,
			method:'post'
		},function(resultData){
			coords = resultData.data;
		});
	}
		...
		for (i=0; i<max; i++) {	
			...
			if(coords!=undefined){
				coords.forEach(function(n){
					if(cluster[i].H.fcltId==String(n.fcltId)){
						var cod =  ol.proj.transform([Number(n.longitude),Number(n.latitude)], 'EPSG:4326', map.getView().getProjection());
						p = cod;
					}
				});
			}
