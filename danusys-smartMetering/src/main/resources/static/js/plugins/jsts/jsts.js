...
/*function move_net(inGid,bufferMiter){
	var f=document.net; 
	var f2=document.net; 
	
	f.inGid.value = inGid; 
	f.bufferMiter.value = bufferMiter; 
	f.action="http://localhost:8080/Sample.do";
	f.method="post"; 
	f.submit(); 
	f2.submit(); 
...
function nwindow() {
	...
	var str = azi.indexOf("{");
	var end = azi.indexOf("}");
	azi_r = azi.substring(str+1,end);
	
	azi_r=azi_r.replace(/\,/g ,'&'); // gi 대소문자 구분x
	// azi_r=azi_r.replace(/\ /g,"");
	azi_r=azi_r.replace(/(\s*)/g,"");
	
	var url='http://localhost:8080/NetMonitor.do';
	
	// window.close();
	window.open(url+"?"+azi_r,"","width=600,height=400,left=600");
