/**
 * 시설물 bar chart 2 생성 관련 함수
 * @namespace facilityBarChart2
 * */

/**
 * 시설물 bar chart 2 생성 함수
 * @function facilityBarChart2.barChart2
 * */
function barChart2() {
	var jsonObj = {};
	$("#barChart2").empty();
	...
	.post(JSON.stringify(jsonObj), function(error, data) {
		...
		bar.append('rect')
			...
			.on("mousemove",function(d) {
				...
				tooltip.html(d.cnt + "건" )
					.style('left', (d3.event.pageX-1265) + "px")
		...
		chart.append('text').attr('text-anchor','middle')
							...
							.text($("#selectBarChart2Type option:selected").text()+" 장애건수");
	});
}