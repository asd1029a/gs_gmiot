...
public class StatisticsSqlProvider {
    ...
    public String selectSumOptQry(Map<String, Object> paramMap) {
        ...
        SQL sql = new SQL() {{
            ...
            INNER_JOIN("v_facility_kind v3 on t2.facility_kind = v3.code_seq");
    ...
    public String selectAvgOptQry(Map<String, Object> paramMap) {
        ...
        SQL sql = new SQL() {{
            SQL subQry1 = new SQL() {{
                ...
                FROM("t_facility_opt t1" +
                        ...
                        "    INNER JOIN t_station t3 on t2.station_seq = t3.station_seq" +
                        "    INNER JOIN v_station_kind v2 on t3.station_kind = v2.code_seq" +
                        "    INNER JOIN v_facility_kind v3 on t2.facility_kind = v3.code_seq");
        ...
        SQL dateSql = new SQL() {{
            SELECT("date_t.t_date AS x_axis, coalesce(min_value, 0) AS min_value, coalesce(max_value, 0) AS max_value, coalesce(avg_value, 0) AS avg_value");
            // 시간대 생성
            String startDtT = "";
            String endDtT = "";
            if (startDt.equals("")) {
