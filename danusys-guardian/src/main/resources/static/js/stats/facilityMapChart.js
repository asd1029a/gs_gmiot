/**
 * 시설물 map chart 관련 모음
 * @namespace facilityMapChart
 * */

/**
 * @memberof facilityMapChart
...
/**
 * 시설물 맵 차트 생성 함수
 * @function facilityMapChart.mapChart
 * @param {string} sigCode - 시군구 코드
 * */
function mapChart(sigCode) {
	$('#mapChart').empty();
	d3.select('#mapChart').selectAll('svg').remove();
	d3.select(".mapTooltip").remove();

	d3.json("/searchDongGis2?sigCode=" + sigCode+"&timeS="+statsMain.sDate+"&timeE="+statsMain.eDate,function (error, geodata) {
		//시군구 코드 및 시군구 명
		if (sigCode == 'all') {
