/**
 * 시설물 heatmap chart 2 생성 관련 함수
 * @namespace facilityHeatmapChart2
 * */

/**
 * 시설물 heatmap chart 2 생성 함수
 * @function facilityHeatmapChart2.blockChart2
 * */
function blockChart2() {
	var jsonObj = {};
	jsonObj.heatmapKind = "f";
	$('#heatmapChart2').empty();
	var yAxis = [];
	var xAxis = [];
	var query;
	...
	if(optVal == "0"){
		query = "facilityChart.facilityHeatMapByTZ"
		jsonObj.kind = "t";
	} else if(optVal == "1"){
		query = "facilityChart.facilityHeatMapByDay"
		jsonObj.kind = "d";
	} else {
		query = "facilityChart.facilityHeatMapByMonth"
		jsonObj.kind = "m";
	}
	...
	.post(JSON.stringify(jsonObj), function(error, data) {
		...
		const chartWidth = $('.stats-area.r .rb').width();
		const chartHeight = $('.stats-area.r .rb').height();
		
		var margin = { top: 50, right: 10, bottom: 50, left: 110 },
		...
		var svg = d3.select("#heatmapChart2").append("svg")
					.attr("width", chartWidth)
					.attr("height", chartHeight)
					.append("g")
					.attr("transform", "translate(" + margin.left + "," + margin.top + ")");
		
		var yLabels = svg.selectAll(".yLabel2")
							.data(yAxis)
							.enter().append("text")
							...
							.attr("class", function (d, i) {
								...
								return "yLabel2 mono r"+i;
							});

		var xLabels = svg.selectAll(".xLabel2")
		...
		var tooltip = d3.select("#heatmapChart2").append("div").attr("class", "heatmapTooltip2");
		...
		var mousemove = function(d) {
            ...
            d3.selectAll(".xLabel2").classed("text-highlight",function(c,ci){ return ci==(d.xtarget-1);});
			//var name;
			d3.selectAll(".yLabel2").classed("text-highlight",function(r,ri){
			...
			/* tooltip.html('<p style="margin:0;font-size:15px;">' + d.ytarget + '</p><br> <div id="heatmapPieArea2"></div>')
					.style("left", (d3.mouse(this)[0]+70) + "px")
					.style("top", (d3.mouse(this)[1]+70) + "px");
			
			if(typeof(d.pieData)!="undefined" && d.pieData.length>0 && d.value>0) {
				d3.select('.heatmapTooltip2').style('width', '430px').style('height', '230px');
				heatmapPieChart(d.pieData);				
			}
			else {
				d3.select('.heatmapTooltip2').style('width', '240px').style('height', '100px');
				d3.select('#heatmapPieArea2').append('div').style('font-size', '20px').text('현재까지 발생한 이벤트 건수가 없습니다.');
			} */
		}
		var mouseleave = function(d) {
			tooltip.style("opacity",0);
			 d3.selectAll(".yLabel2").classed("text-highlight",false);
             d3.selectAll(".xLabel2").classed("text-highlight",false);
		}
		svg.append('text').attr('text-anchor','middle')
			.attr('transform', 'translate('+(width/2-30)+","+(height+40)+')')
			.style({'font-size':'13px', 'fill': '#cecece'})
			.text($("#selectBarChart2Type option:selected").text()+"-용도별 장애건수 히트맵");
		svg.selectAll(".block")
		...
		function heatmapPieChart(makedata) {
			...
			//파이위치
			var c3_g = d3.select('#heatmapPieArea2').append('svg')
			...
			d3.select("#heatmapPieArea2").append('svg').attr('class', 'legendBack')
				.style('background', 'none').style('position', 'absolute').style('height', '100%');
			
			//파이 범례
			var legend = d3.select('#heatmapPieArea2').select('.legendBack').selectAll('.legend')
